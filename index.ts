...
<div class="container">
  ...
  <div class="anonymity-badge"><strong>🔒 Анонимный опрос</strong></div>
...
<script>
  ...
  document.getElementById('surveyForm').addEventListener('submit', async (e) => {
    ...
    if (blocks.length === 0) {
      ...
      document.getElementById('result').innerHTML = '❌ Добавьте хотя бы одного коллегу.';
      return;
    }
    ...
    if (Object.keys(data).length < 3) {
      document.getElementById('result').className = 'result error';
      document.getElementById('result').innerHTML = '❌ Ответьте на все вопросы.';
      return;
    }
    ...
    try {
      ...
      if (result.status === 'success') {
        ...
        resultDiv.innerHTML = '<h3>✅ Спасибо за ответы!</h3><p>Данные сохранены.</p>';
