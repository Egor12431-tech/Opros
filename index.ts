...
try {
  ...
  // Продолжаем работу, но без базы данных
}
...
<script>
  ...
  function renderColleagueBlock(index, name) {
    ...
    sectionBQuestions.forEach((q, qi) => {
      ...
      html += '<div class="question-block" style="border-left-color:#0077b6;"><div class="qtext">' + (qi+1) + '. ' + q.text + ' <span style="color:#999;font-size:12px;">[' + label + (q.reverse ? ' ⚠️ обратная шкала' : '') + ']</span></div><div class="options">';
      for (let v = 1; v <= 5; v++) {
  ...
  document.getElementById('surveyForm').addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (result.status === 'success') {
        ...
        resultDiv.innerHTML = '<h3>✅ Спасибо за ответы!</h3><p>Ваши ответы успешно сохранены.</p><p style="color:#666;font-size:14px;">Все оценки коллег будут обработаны и представлены на итоговом совещании.</p>';
...
app.post("/api/submit", async (c) => {
  try {
    ...
    if (!pool) {
      return c.json({ status: "error", message: "База данных не подключена" }, 500);
    }
    ...
    for (const col of colleagues) {
      ...
      const existing = await pool.query("SELECT * FROM colleague_aggregates WHERE colleague_name = $1", [name]);
    ...
    return c.json({ status: "success", id: Date.now() });
  } catch (error: any) {
